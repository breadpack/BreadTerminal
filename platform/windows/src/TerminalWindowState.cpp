...
void TerminalWindowState::initTerminal() {
    ...
    agentTreeTracker = std::make_unique<termcore::AgentTreeTracker>();
    hookBridge = std::make_unique<termcore::HookBridge>(
        *agentTreeTracker, *agentTracker, *notifications);
    ...
    // Wire OSC 7770 callback for all screens (current + future tabs/splits).
    // Routes all in-band events through HookBridge.
    {
        auto* bridge = hookBridge.get();
        auto oscCallback = [bridge](const std::string& json_str) {
            auto j = nlohmann::json::parse(json_str, nullptr, false);
            if (j.is_discarded() || !j.is_object()) return;
            bridge->processHookEvent(j);
        };

        // Apply to the initial screen
        termcore::Screen* scr = controller->activeScreen();
        if (scr) {
            scr->setOscHookCallback(oscCallback);
        }

        // Register callback so all future screens get the same wiring
        controller->addScreenCreatedCallback(
            [oscCallback = std::move(oscCallback)](termcore::Screen* screen) {
                screen->setOscHookCallback(oscCallback);
            });
    }

    // Create SidebarModel (needs AgentTreeTracker, created after controller)
    sidebarModel = std::make_unique<termcore::SidebarModel>(*agentTreeTracker);
