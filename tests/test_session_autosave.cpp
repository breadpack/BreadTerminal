...
TEST(SessionAutoSave, SaveLoadRoundtrip) {
    ...
    termcore::SessionAutoSave autosave;
    autosave.saveNow(original);  // save_path_ is empty, should be no-op

    // Use the static-style API with explicit path
    termcore::AutoSaveConfig config;
    config.save_path = dir.path();
    autosave.start(config, [&]() { return original; });
    autosave.stop();

    // saveNow with a configured path
    {
        termcore::SessionAutoSave saver;
        // Manually set up by starting and immediately stopping, then saveNow
        // Actually, saveNow uses save_path_ which is set by start().
        // Let's use start + saveNow approach:
        saver.start(
            {true, 9999, dir.path()}, [&]() { return original; });
...
// ---------------------------------------------------------------------------
// Test: atomic write produces a .tmp file that is renamed
// ---------------------------------------------------------------------------
